...
local boostSpeed = 1
local boostDelay = 0.1

local qeMacroEnabled = false
local qeDelay = 0.035
...
local function getLeaderAhead()
    ...
    return bestPlr, bestDot
end

local function startQEMacro()
    task.spawn(function()
        while qeMacroEnabled do
            if hrp and hum and hum.MoveDirection.Magnitude > 0 then
                VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.Q, false, game)
                task.wait(qeDelay * 0.4)
                ...
                task.wait(qeDelay + math.random(-4, 4) / 1000)
            else
                task.wait(0.05)
            end
        end
...
local Window = Library:CreateWindow({ Title = "Lyzn Hub", SubTitle = "Track And Field" })

local Movement = Window:CreateTab("Movement", "run")

local boostSection = Movement:CreateSection("CFrame Boost", "Left")
boostSection:AddToggle({ Text = "CFrame Boost", Default = false, Callback = function(v)
    boostEnabled = v
    if v then
        task.spawn(function()
            while boostEnabled do
                if hrp and hum and hum.MoveDirection.Magnitude > 0 then
                    hrp.CFrame = hrp.CFrame + hum.MoveDirection.Unit * boostSpeed
                end
                task.wait(boostDelay)
            end
        end)
    end
end })
boostSection:AddSlider({ Text = "Boost Speed", Min = 0.1, Max = 15, Default = 1, Decimals = 1, Callback = function(v) boostSpeed = v end })
boostSection:AddSlider({ Text = "Boost Delay", Min = 0.01, Max = 1, Default = 0.1, Decimals = 2, Callback = function(v) boostDelay = v end })

local qeSection = Movement:CreateSection("Q/E Sprint Macro", "Right")
qeSection:AddToggle({ Text = "Q/E Macro", Default = false, Callback = function(v)
    qeMacroEnabled = v
    if v then startQEMacro() end
end })
qeSection:AddSlider({ Text = "Macro Speed", Min = 0.01, Max = 0.12, Default = 0.035, Decimals = 3, Callback = function(v) qeDelay = v end })

local Misc = Window:CreateTab("Misc")

local afkSection = Misc:CreateSection("Anti AFK", "Left")
afkSection:AddToggle({ Text = "Anti AFK", Default = false, Callback = function(v)
    antiAfkEnabled = v
    if v then
        task.spawn(function()
...
print("Lyzn Hub - Track And Field loaded")